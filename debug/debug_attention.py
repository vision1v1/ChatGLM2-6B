...
if __name__ == '__main__':
    ...
    def print_outputs(outputs):
        context_hidden_states , attn_weights, past_key_value = outputs
        print(f"context_hidden_states {list(context_hidden_states.shape) if context_hidden_states is not None else []}", context_hidden_states, sep='\n', end='\n\n')
        print(f"attn_weights {list(attn_weights.shape) if attn_weights is not None else []}", attn_weights, sep='\n', end='\n\n')
        print(f"past_key_value {list(past_key_value.shape) if past_key_value is not None else []}", past_key_value, sep='\n', end='\n\n')

    attn = MultiQueryAttention(768, 8, 'torch')

    # attn = MultiheadAttention(768, 8, 'torch')

    outputs = attn.forward(torch.ones(size=(1, 512, 768)))
